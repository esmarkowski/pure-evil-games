...
export class Forest extends View {
    ...
    init(pineCount = 20, deciduousCount = 20, width) {
            ...
            for(var i = points.length; i < pineCount + deciduousCount; i++) {
            ...
            for(let polygon of cells) {
                ...
                voronoiMesh.position.y = 5
                // this.add(voronoiMesh)
            }
    ...
    addTree(site) {
        ...
        switch(site.type) {
            case "Pine":
               tree = Pine.init(1, radius, height, 0x99d200);
               break;
            case "Deciduous":
                tree = Deciduous.init(1, radius, height * 1.6, 0x71b300);
                break;
        }

        tree.position.x = site.x;
        tree.position.z = site.z;
        tree.position.y = 0;
